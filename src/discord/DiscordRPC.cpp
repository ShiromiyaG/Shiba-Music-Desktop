                                ...
                                qint64 position, qint64 duration)
{
    ...
    if (title == m_lastTitle && artist == m_lastArtist &&
        album == m_lastAlbum && playing == m_lastPlaying &&
        qAbs(position - m_lastPosition) < 2000)
    {
        return;
    }
    ...
    if (!heading.isEmpty())
    {
        ...
        activity.insert(QStringLiteral("status_display_type"), 0); // Show "name" in the header
    }
    ...
    if (playing && duration > 0)
    {
        ...
        const qint64 start = nowMs - position;
        const qint64 end = start + duration;
        QJsonObject timestamps;
        timestamps.insert(QStringLiteral("start"), start);
        timestamps.insert(QStringLiteral("end"), end);
